...
if __name__ == "__main__":
    ...
    idx, best_recalln2_result = max(
        enumerate(results), key=lambda x: x[1]["recall"][-2]
    )
    ...
    # best recall at timeshift<320 if fp < 1.2*259
    best_fp_result = max(
        ((idx, x) for idx, x in enumerate(results) if x["fp"][-2] < 1.2 * 259),
        key=lambda x: x[1]["recall"][-2],
        default=None,
    )
    if best_fp_result:
        best_fp_index, best_fp_data = best_fp_result
        # print(idx, f"best recall and fp < 1.2*259: {best_fp_data}")
        print("Exp4: best recall if fp < 1.2*259")
    ...
    best_fp_result = min(
        ((idx, x) for idx, x in enumerate(results) if x["recall"][-2] * 100 > 97 * 0.8),
        key=lambda x: x[1]["fp"][-2],
        default=None,
    )
    if best_fp_result:
        best_fp_index, best_fp_data = best_fp_result
    ...
    best_fp_result = max(
        ((idx, x) for idx, x in enumerate(results) if x["fp"][-2] < 1),
        key=lambda x: x[1]["recall"][-2],
        default=None,
    )
    if best_fp_result:
    ...
    # [60, 720, 3.5, 0.75] 97.37 987
    # [160, 680, 3.25, 0.75] 68.42 301
    # [100, 760, 3.75, 1.0] 78.95 488
    # Exp5 [60, 720, 3.5, 0.75]
    specific_params_result = next(
        (
            (idx, x)
            for idx, x in enumerate(results)
            if x["params"] == [60, 720, 3.5, 0.75]
        ),
        None,
    )
    if specific_params_result:
        specific_params_index, specific_params_data = specific_params_result
        print("Exp5: candidate algo params [60, 720, 3.5, 0.75]")
        print("params", specific_params_data["params"])
        print("score", specific_params_data["score"])
        print("recall%", specific_params_data["recall"][-2] * 100)
        print("fp", specific_params_data["fp"][-2])
    ...
    # [160, 680, 3.25, 0.75] 68.42 301
    # [100, 760, 3.75, 1.0] 78.95 488
    # Exp6 [160, 680, 3.25, 0.75]
    specific_params_result = next(
        (
            (idx, x)
            for idx, x in enumerate(results)
            if x["params"] == [160, 680, 3.25, 0.75]
        ),
        None,
    )
    if specific_params_result:
        specific_params_index, specific_params_data = specific_params_result
        print("Exp6: candidate algo params [160, 680, 3.25, 0.75]")
        print("params", specific_params_data["params"])
        print("score", specific_params_data["score"])
        print("recall%", specific_params_data["recall"][-2] * 100)
        print("fp", specific_params_data["fp"][-2])
    ...
    # [160, 680, 3.25, 0.75] 68.42 301
    # [100, 760, 3.75, 1.0] 78.95 488
    # Exp7 [100, 760, 3.75, 1.0]
    specific_params_result = next(
        (
            (idx, x)
            for idx, x in enumerate(results)
            if x["params"] == [100, 760, 3.75, 1.0]
        ),
        None,
    )
    if specific_params_result:
        specific_params_index, specific_params_data = specific_params_result
        print("Exp7: candidate algo params [60, 560, 3.25, 0.75]")
        print("params", specific_params_data["params"])
        print("score", specific_params_data["score"])
        print("recall%", specific_params_data["recall"][-2] * 100)
        print("fp", specific_params_data["fp"][-2])
