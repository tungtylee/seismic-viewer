...
def viewer_all(tr, tr_data, figtitle, arrivallist_in_s, figfn=None):
    tr_times = tr.times()
    # Initialize figure
...
if __name__ == "__main__":
    ...
    for idx, fn in enumerate(fnlist):
        subfnlist = [fn]
        for m in range(4):
            sta_len, lta_len, thr_on, thr_off = sta_len_v[m], lta_len_v[m], thr_on_v[m], thr_off_v[m]
            fnamelist, time_abs_str_list, time_rel_list, alltr, alltrdata = \
                eval.run_stalta_from_fnlist(data_directory, subfnlist, sta_len, lta_len, thr_on, thr_off, ext=True)
